...
for digit in list(range(0,10)):
    ...
    for i, image in enumerate(images):
        ...
        newimages_labels.append(j)
        newimages.append(
            rotate(image, j)
        ) 
    if os.path.exists('MNIST_figures/M{}.p'.format(digit)):
        #save time by loading the distance matrix if it exists
        M = pickle.load(open('MNIST_figures/M{}.p'.format(digit), 'rb'))
    else:
        print('Computing M...')
        M = distance_matrix(newimages, newimages)
        pickle.dump(M, open('MNIST_figures/M{}.p'.format(digit), 'wb'))
    X = {}
    y = {}
    r = {}
    times = {}
    ...
    for m in R2_methods+circle_methods:
        ...
        if m == 'Isomap':
            ...
            print('Isomap', end ='|')
            start_time = time.time()
            X['Isomap'] = Isomap(n_components=2, tol=1e-4).fit_transform(np.array(newimages))
